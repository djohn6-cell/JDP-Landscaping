...
const services = [
  { name: "Trimming", icon: <TrimIcon /> },
  { name: "Mulching", icon: <MulchIcon /> },
  { name: "Removal", icon: <RemoveIcon /> },
  { name: "Planting", icon: <PlantIcon /> },
  { name: "Cleanups", icon: <CleanupIcon /> },
];
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="text-center max-w-2xl mx-auto mb-12">
          ...
          <p className="text-brand-charcoal/70 text-base sm:text-lg">
            From routine maintenance to full property transformations — we
            handle it all.
          </p>
        </div>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-4 mb-10">
          {services.map((service, i) => (
            <div
              key={service.name}
              className={`bg-white rounded-2xl p-6 shadow-sm border border-black/5 text-center group hover:shadow-md transition-shadow duration-200${i === services.length - 1 && services.length % 2 !== 0 ? " col-span-2 sm:col-span-1" : ""}`}
            >
              <div className="w-12 h-12 bg-brand-green/10 group-hover:bg-brand-green/20 rounded-xl flex items-center justify-center text-brand-green mx-auto mb-3 transition-colors">
                {service.icon}
              </div>
